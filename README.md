...
| 💰 AI Capital OS | 资本总入口：资本进入 → AI 理解资本 → 配置 → 寻找机会 → 自动投资 AI 企业 → AI 劳动力生产 → 服务收入 → 利润回资本 → 再平衡，九环节闭环实时运行 | 导航「💰 CAPITAL」· 首页横幅 · 欢迎页主按钮 |

## AI Capital OS（💰 资本总入口）

导航首位「💰 CAPITAL 总入口」或首页横幅进入。三步入口：**你的资本 → 你的目标 → AI Capital Plan**，AI Capital Agent 按「资本规模 + 投资目标」生成板块配置、推荐资产与再平衡计划；点击「启动 AI Capital OS 闭环」后：

1. 账户重置为所选资本规模（保留 WEG 与 AI 信用）；
2. AI **自动买入**方案推荐资产（92% 部署，8% 现金储备，生成真实持仓）；
3. 闭环引擎每 2.5 秒按真实市场状态运行：**AI 劳动力生产 → 服务收入（30% 利润率）→ 60% 分红回流账户现金 / 40% 留存再投资 → 配置偏离 >10% 或情绪极端时自动再平衡**；
4. CAPITAL 页切换为「闭环驾驶舱」：九环节实时状态、组合净值曲线、利润回流曲线、目标配置条与运行年化（按真实运行时间折算）。

实现位于 `src/ai/capitalOs.ts`（纯函数闭环引擎）+ `src/pages/Capital.tsx`（驾驶舱）+ `src/store/market.ts`（tick 驱动接入）。
